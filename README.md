...
- [技术交付文档](docs/technical-delivery.md)：总体架构、并发治理、高可用、负载均衡、容错恢复和技术取舍。
...
DocLens-j 将上传接入、文档准备、页级 OCR、节点路由、结果聚合和回调投递拆成独立边界。
高并发场景下，它通过短事务落库、文档线程池、数据库页任务、worker 抢占锁、OCR 节点槽位、
pending queue、健康探测、熔断、重试和过期任务恢复来保证任务可调度、可恢复、可解释。

完整的技术能力、高可用设计和技术取舍见 [技术交付文档](docs/technical-delivery.md)。
