...
public record OcrNodeResponse(
    ...
    public static OcrNodeResponse from(OcrNode node, OcrNodeMetrics metrics) {
        return new OcrNodeResponse(node.id(), node.modelKey(), node.deploymentType().name(), node.name(), node.host(),
                ...
                node.credentialRef().orElse(""), node.credentialConfigured(), node.enabled(),
                node.participateGlobal(), node.weight(), node.maxConcurrency(), node.status().name(),
                metrics.inflightImages(), metrics.queuedImages(), metrics.processedImagesToday(),
                metrics.successImages(), metrics.failedImages(), metrics.avgLatencyMs(),
