...
- [ ] 1.1 Add a failing test that proves a large Markdown document submits all
...
- [ ] 1.2 Add a failing test that proves two documents can share the same chunk
...
- [ ] 1.3 Implement a chunk execution coordinator that submits each chunk as an
...
- [ ] 1.4 Extend the chunk execution tests so a chunk that exhausts retries falls
  back to its original chunk text while the remaining chunks still complete and
  merge normally.
- [ ] 1.5 Re-run the focused backend tests for Markdown chunking and the broader
