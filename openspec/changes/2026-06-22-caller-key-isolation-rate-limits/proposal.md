...
“凭证认证”和“数据隔离”之间。用户确认后的真实目标不是让 DocLens
判断这个 key 属于谁，而是让调用方传入一个不透明分区键：
...
- Dashboard 继续从 `sessionStorage` 读取 `X-DocLens-Credential-Key`，但
  请求时原样发送同名 header，不再转换为 `X-DocLens-Api-Key` 或
  `Authorization`。
- 后端读取 `X-DocLens-Credential-Key` 作为 opaque caller partition key。
...
- 任意非空 `X-DocLens-Credential-Key` 都可进入业务路径。
